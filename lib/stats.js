 ...
 */
class StatsReporter {
  constructor({ webApiUrl, nodeToken, onEnforcement }) {
    this.webApiUrl    = webApiUrl;
    this.nodeToken    = nodeToken;
    this.onEnforcement = onEnforcement;
  ...
  async _send(keyId, rx, tx) {
    try {
      const response = await axios.post(
        ...
        {
          headers: { "x-node-token": this.nodeToken, "content-type": "application/json" },
          timeout: 5000,
        }
      );

      if (response.data?.shouldDisconnect && this.onEnforcement) {
        await this.onEnforcement({
          keyId,
          reason: response.data.reason || "quota_exceeded",
        });
      }
    } catch (err) {
      const status = err.response?.status;
      // 404 = key ถูกลบไปแล้ว ไม่ต้อง retry อีก
      if (status === 404) {
        logger.warn("Key no longer exists, discarding usage data", { keyId });
        return;
      }
      logger.error("Failed to report usage", { keyId, error: err.message });
      // ใส่กลับ pending เพื่อ retry รอบหน้า (เฉพาะ error ชั่วคราว)
      const cur = this.pending.get(keyId) || { rx: BigInt(0), tx: BigInt(0) };
