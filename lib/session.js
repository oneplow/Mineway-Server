...
class TunnelSession extends EventEmitter {
  constructor({ ws, keyId, userId, tunnelId, assignedPort, plan, maxPlayers, reporter }) {
    super();
    this.ws           = ws;
    this.keyId        = keyId;
    this.userId       = userId;
  ...
  async start() {
    ...
    // Periodically drain accumulated bandwidth to the reporter (every 30s)
    this._statsDrain = setInterval(() => this._drainStats(), 30_000);
    this._statsDrain.unref();
  ...
  destroy(reason = "unknown") {
    ...
    clearInterval(this._statsDrain);
  ...
  _bindTCP() {
    return new Promise((resolve, reject) => {
      ...
      this._tcpServer.on("error", (err) => {
        // After initial bind, log but don't crash
  ...
  _onTCP(sock) {
    if (this.suspended) { sock.destroy(); return; }
    ...
    // Set socket timeout to clean up idle connections (5 min)
    sock.setTimeout(5 * 60_000, () => {
      logger.debug("TCP socket timed out", { connId });
      sock.destroy();
    });
    ...
    sock.on("error", (err) => {
      logger.debug("TCP socket error", { connId, error: err.message });
      sock.destroy();
    });
  }
  ...
  _bindUDP() {
    return new Promise((resolve, reject) => {
      ...
      this._udpSocket.on("message", (msg, rinfo) => {
        try {
          this._onUDP(msg, rinfo);
        } catch (err) {
          logger.debug("UDP message handler error", { keyId: this.keyId, error: err.message });
        }
      });
  ...
  /**
   * Drain accumulated bandwidth to the reporter and reset counters.
   * Called every 30s so the dashboard shows near-real-time traffic.
   */
  _drainStats() {
    if (this.rxBytes === BigInt(0) && this.txBytes === BigInt(0)) return;
    const rx = this.rxBytes;
    const tx = this.txBytes;
    this.rxBytes = BigInt(0);
    this.txBytes = BigInt(0);
    if (this.reporter) {
      this.reporter.record(this.keyId, rx, tx);
    }
  }
